...
import getpass
import numpy as np
from hvmodbus_test import HVModbus
...
class HighVoltageApp(cmd2.Cmd):
    ...
    def printMonitorHeader(self):
        self.poutput(cmd2.ansi.style(self.st.generate_data_row(['','status','DAC','V_5','V3.3','T','rate UP/DN','limit V/I/T/TRIP','trigger thr','alarm']), fg=cmd2.ansi.Fg.LIGHT_CYAN))
        self.poutput(cmd2.ansi.style(self.st.generate_data_row(['','','[lsb]','[V]','[V]','[°C]','[V/s]/[V/s]','[V]/[uA]/[°C]/[s]','[mV]','']), fg=cmd2.ansi.Fg.LIGHT_BLUE))
    ...
    # Pulse PreScaler 
    #
    Presc_parser = argparse.ArgumentParser()
    Presc_parser.add_argument('value', type=int, help='voltage level [V] (min:25 max:1500)')

    @cmd2.with_argparser(Presc_parser)
    @cmd2.with_category("High Voltage commands")
    def do_Presc(self, args: argparse.Namespace) -> None:
        """Set voltage"""
        if self.checkConnection() is False:
            return
        self.hv.setPulseFreq(args.value)

    #
    # DAC voltage transparent 
    #
    DACvoltage_parser = argparse.ArgumentParser()
    DACvoltage_parser.add_argument('value', type=int, help='voltage level [V] (min:25 max:1500)')

    @cmd2.with_argparser(DACvoltage_parser)
    @cmd2.with_category("High Voltage commands")
    def do_DACvoltage(self, args: argparse.Namespace) -> None:
        """Set voltage"""
        if self.checkConnection() is False:
            return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--port', action='store', type=str, help='serial port device (default: /dev/ttyPS2)', default='COM6')
