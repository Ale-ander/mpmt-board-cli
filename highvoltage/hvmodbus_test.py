...
class HVModbus:
   ...
   def getVoltageSet(self, slave=None):
      slave = self.address if slave is None else slave
      rr = self.client.read_holding_registers(address=0x21, count=1, slave=slave)
      return rr.registers[0]

   def setVoltageSet(self, value, slave=None):
      slave = self.address if slave is None else slave
      self.client.write_register(address=0x21, value=value, slave=slave)

   def setPulseFreq(self, value, slave=None):
      slave = self.address if slave is None else slave
      self.client.write_register(address=0x36, value=value, slave=slave)
   ...
   def powerOn(self, slave=None):
      slave = self.address if slave is None else slave
      rr = self.client.write_coil(address=3, value=True, slave=slave)
      return not rr.isError()

   def powerOff(self, slave=None):
      slave = self.address if slave is None else slave
      rr = self.client.write_coil(address=3, value=False, slave=slave)
      return not rr.isError()
   ...
   def readMonRegisters(self, slave=None):
      ...
      monData['Vset'] = rr.registers[0x0021]
